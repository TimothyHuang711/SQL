...
-- US Region Revenue Contribution By Streamer Level
WITH
  DateRange AS (
    SELECT
      DATE("2022-05-01") AS startDate, 
      DATE("2022-09-21") AS endDate
  )
  ,Income AS (
    SELECT
        DATE(
    ...
    WHERE
        DATE(
            PUI.timestamp_utc,
            PUI.receiver_inferStreamerTimezone
        ) BETWEEN (SELECT startDate FROM DateRange) AND (SELECT endDate FROM DateRange)
        AND DATE_SUB(
            DATE(PUI.timestamp_utc, 'Asia/Taipei'),
            INTERVAL 1 DAY
        ) < CURRENT_DATE('Asia/Taipei')
        ...
        AND receiver_inferStreamerRegionGroup IN("United States", "Australia", "Canada", "Colombia", "Mexico")
    GROUP BY
        tzDate,
...
SELECT DISTINCT
  ...
  operationRegionGroup,
  maxlevel,
  SUM(dailyReceivedPrice) OVER (PARTITION BY 
    DATE_TRUNC(tzDate, MONTH), 
    operationRegionGroup,
    maxlevel
  ) AS revenueByLevel,
  SUM(dailyReceivedPrice) OVER (PARTITION BY 
...
FROM IncomeWithLevel
ORDER BY month, operationRegionGroup, maxlevel